...
\abstractCS{
Tato práce realizuje iOS aplikaci pro evidenci letů.  V~první části jsou rozebrány různé možnosti architektur a uložení dat při tvorbě iOS aplikací společně s~popisem funkcionálně reaktivního programování.

V~další části jsou analyzovány obdobné aplikace a předpisy pro piloty České republiky, podle nichž probíhá návrh funkcionality vytvářené aplikace. Podle návrhu je následně zvolena vhodná architektura a frameworky a je vytvořeno uživatelského rozhraní v~podobě wireframů, které slouží jako podklad pro samotnou implementaci aplikace.
...
\abstractEN{
This thesis implements an iOS application for flights recording. In the first part software architectures and data persistence solutions in the iOS development are analyzed along with funcional reactive programming characterization.

Next I~perform analysis of similar applications and regulations for pilots of the Czech Republic. According to which I~design functionalities, choose the appropriate architecture and frameworks and create the user interface in a form of wireframes. These wireframes provide the basis for an impementation.

The application is implemented with the selected architecture and the FRP. During the impelemetation, unit tests are created and at the end user tests are performed. These tets help to adjust the application to its final version.

In the last part I~describe advantages and disadvantages which caused by FRP procedures. I~also evaluate time and implementation difficulties compared to standard procedures and the classical MVC archicture.
 
In this thesis I~create the functional iOS application using the modern architecture and FRP. This application will help its users to record and manage their flights.

In the appendix of this thesis, all source files for both application and tests can be found along with documentation, wireframes and user tests recordings.
}
...
\begin{introduction}
V~dnešní době, kdy existují mobilní aplikace na téměř vše, mě zarazil fakt, že u~pilotů tomu tak nemusí být. Aplikace na evidenci letů samozřejmě existují, však je tu hned několik problémů. Tyto aplikace jsou často velice drahé, nemusí odpovídat leteckým přepisům České republiky (resp. Evropské Unie) nebo nemají vyhovující funkcionalitu.
...
Dalším tématem, které ve své práci řeším, jsou softwarové architektury při vývoji iOS aplikací. Zde analyzuji alternativy k~architektuře MVC ve spojení s~funkcionálně reaktivním programováním neboli FRP.
